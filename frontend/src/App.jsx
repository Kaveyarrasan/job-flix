...
const HomePage = ({ trendingJobs, remoteJobs, setSelectedJob, handleApply }) => (
  <div className="animate-fadeIn">
    ...
    <header className="relative h-[85vh] w-full bg-cover bg-center flex items-center px-6 md:px-12" style={{ backgroundImage: 'linear-gradient(to right, #ffffff 20%, transparent), url("https://images.unsplash.com/photo-1497215728101-856f4ea42174?auto=format&fit=crop&q=80&w=2070")' }}>
      <div className="max-w-3xl space-y-6 pt-32 pb-20 relative z-10">
        <h2 className="text-netflix-red font-bold uppercase tracking-[0.5em] text-xs md:text-sm">Featured Role</h2>
        <h1 className="text-4xl md:text-7xl font-black leading-[1.1] drop-shadow-sm text-gray-900">Senior Full <br /> Stack Developer</h1>
        <p className="text-lg md:text-xl text-gray-700 max-w-xl font-medium">
        ...
        <div className="flex space-x-4 pt-6">
          <button 
            ...
            className="bg-gray-900 text-white px-6 md:px-10 py-3 rounded-md font-bold hover:bg-black transition flex items-center space-x-2 shadow-xl"
          >
            <span className="text-xl">▶</span> <span>Apply Now</span>
          </button>
          <button className="bg-gray-100 text-gray-900 px-6 md:px-10 py-3 rounded-md font-bold hover:bg-gray-200 transition flex items-center space-x-2 border border-gray-200 shadow-sm">
            <span className="text-lg text-gray-600">ⓘ</span> <span>More Info</span>
          </button>
        </div>
      </div>
      <div className="absolute bottom-0 left-0 w-full h-64 bg-gradient-to-t from-white via-white/50 to-transparent"></div>
    </header>

    {/* Content Rows with White Background */}
    <main className="relative z-40 pb-20 space-y-16 bg-white -mt-4 md:-mt-8">
...
const App = () => {
  ...
  return (
    <Router>
      <div className="min-h-screen bg-white text-gray-900 overflow-x-hidden selection:bg-netflix-red selection:text-white">
        <Navbar />
        ...
        {/* Global Footer - Brighter */}
        <footer className="bg-gray-50 border-t border-gray-200 px-6 md:px-12 py-10 text-gray-500 text-sm">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-6xl mx-auto">
            <div className="space-y-3">
              <p className="hover:text-black cursor-pointer transition-colors">Help Center</p>
              <p className="hover:text-black cursor-pointer transition-colors">Terms of Use</p>
            </div>
            <div className="space-y-3">
              <p className="hover:text-black cursor-pointer transition-colors">Privacy</p>
              <p className="hover:text-black cursor-pointer transition-colors">Cookie Preferences</p>
            </div>
          </div>
          <p className="mt-10 text-center text-gray-400 font-medium tracking-wide">© 2024-2026 JOBFLIX PORTAL</p>
        </footer>
      </div>
    </Router>
  );
